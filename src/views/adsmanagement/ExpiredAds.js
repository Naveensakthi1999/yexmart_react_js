...
import { adFormData, adTableData } from '../../components/form-field';
...
const ExpiredAds = () => {
  ...
  const [tableData, setTableData] = useState(adTableData.filter(ad => ad.status === 'expired'));
  const [formData, setFormData] = useState('');
  const [search, setSearch] = useState('');
  const [errors, setErrors] = useState({});
  const [isEdit, setIsEdit] = useState({ status: false, editId: '' });

  const BCrumb = useMemo(() => [{ to: '/', title: 'Home' }, { title: 'Expired Ads' }], []);
  ...
  return (
    <PageContainer title="Expired Ads Details" description="this is Expired Ads Form page">
      <Breadcrumb title="Expired Ads Information" items={BCrumb} />
...
export default React.memo(ExpiredAds);
