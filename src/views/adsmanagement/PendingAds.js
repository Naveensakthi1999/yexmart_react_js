...
const PendingAds = () => {
  ...
  const [tableData, setTableData] = useState( adTableData.filter(ad => ad.status === 'pending'));
  const [formData, setFormData] = useState('');
  const [search, setSearch] = useState('');
  const [errors, setErrors] = useState({});
  const [isEdit, setIsEdit] = useState({ status: false, editId: '' });

  const BCrumb = useMemo(() => [{ to: '/', title: 'Home' }, { title: 'Pending Ads' }], []);

  const handleDownload = useCallback(() => {
    const headers = ['S.No', 'Pending Ads'];
  ...
  return (
    <PageContainer title="Pending Ads Details" description="this is Pendung Ads Form page">
      <Breadcrumb title="Pending Ads Information" items={BCrumb} />
...
export default React.memo(PendingAds);
