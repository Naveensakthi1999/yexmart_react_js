...
import { unitFormData, adTableData } from '../../components/form-field';
...
const ReportedAds = () => {
  ...
  const [tableData, setTableData] = useState(adTableData.filter(ad => ad.status === 'report'));
  const [formData, setFormData] = useState('');
  const [search, setSearch] = useState('');
  const [errors, setErrors] = useState({});
  const [isEdit, setIsEdit] = useState({ status: false, editId: '' });

  const BCrumb = useMemo(() => [{ to: '/', title: 'Home' }, { title: 'Reported Ads' }], []);
  ...
  return (
    <PageContainer title="Reported Ads Details" description="this is Reported Ads Form page">
      <Breadcrumb title="Reported Ads Information" items={BCrumb} />
...
export default React.memo(ReportedAds);
