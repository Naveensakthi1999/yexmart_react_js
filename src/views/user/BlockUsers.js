...
const BlockUsers = () => {
  ...
  const getUsers = async (page, size, search = '') => {
    try {
      ...
      if (response.data?.status) {
        ...
        const blockedUser = user.filter((u) => u.status === false);
        setTableData(blockedUser);
      }
  ...
  return (
    <>
      <PageContainer title="Block Users Details" description="this is User Form page">
        <Breadcrumb title="Block Users Information" items={BCrumb} />
...
export default React.memo(BlockUsers);
