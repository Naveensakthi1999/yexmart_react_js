...
import { paymentTypeTableData, paymentTypeFormData } from '../../components/form-field';
...
const PaymentTypeDetails = () => {
  ...
  const [tableData, setTableData] = useState(paymentTypeTableData);
  const [formData, setFormData] = useState('');
  const [search, setSearch] = useState('');
  const [errors, setErrors] = useState({});
  const [isEdit, setIsEdit] = useState({ status: false, editId: '' });

  const BCrumb = useMemo(() => [{ to: '/', title: 'Home' }, { title: 'Payment Type List' }], []);

  const handleDownload = useCallback(() => {
    const headers = ['S.No', 'Payment Type'];
    const rows = tableData.map((item, index) => [index + 1, item.payment_type_title]);
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('payment_type_title', {
        header: () => 'Payment Type Title',
  ...
  return (
    <PageContainer title="Payment Type Details" description="this is Payment Type Form page">
      <Breadcrumb title="Payment Type Information" items={BCrumb} />
...
export default React.memo(PaymentTypeDetails);
